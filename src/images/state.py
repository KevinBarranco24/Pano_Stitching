...
class State:
  ...
  def _calculate_params(self, cameras):
    self._params = np.zeros((constants.PARAMS_PER_CAMERA * len(cameras)), dtype=np.float64)

    for i in range(0,  len(cameras) * constants.PARAMS_PER_CAMERA, constants.PARAMS_PER_CAMERA):
      camera = cameras[i//constants.PARAMS_PER_CAMERA]
