...
from src.images.state import State
from src.images.constants import PARAMS_PER_CAMERA, PARAMS_PER_POINT_MATCH, REGULARISATION_PARAM, MAX_ITR
...
class BundleAdjuster:
  ...
  def run(self):
    ...
    #CostFunction = math.sqrt(np.mean(next_residuals**2))#FUNCION PERRONA
    VarMin = -5
    VarMax = 5
    nVar = 1
    ...
    for i in range(0, PopSize):
        ...
        pop[i]['position'] = next_residuals
        pop[i]['velocity'] = np.zeros(nVar)
        pop[i]['cost'] = math.sqrt(np.mean(pop[i]['position']**2))
        pop[i]['best_position'] = pop[i]['position'].copy()
        pop[i]['best_cost'] = pop[i]['cost']
        pop[i]['best_state'] = next_state #yo la AÑADI BECAUSE YES
    ...
    for it in range(0, MaxIter):
        for i in range(0, PopSize):
            ...
            if pop[i]['cost'] < pop[i]['best_cost']:
                pop[i]['best_position'] = pop[i]['position'].copy()
                pop[i]['best_cost'] = pop[i]['cost']
                best_state = pop[i]['best_state']
                best_error = pop[i]['cost'] 
  ...
  def _projection_errors(self, state):
    ...
    num_pointwise_matches = sum(len(match.matches) for match in self._matches)
